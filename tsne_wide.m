% t-SNE visualizations for Wide Network
clc;
clear;
...
title('t-SNE - train - Wide');
saveas(train_plot, 'tsne_train_wide.png')

val_plot = figure;
gscatter(Yval_tsne(:,1), Yval_tsne(:,2), val.Labels);
title('t-SNE - Val - Wide');
saveas(val_plot, 'tsne_val_wide.png')

test_plot = figure;
gscatter(Ytest_tsne(:,1), Ytest_tsne(:,2), test.Labels);
title('t-SNE - Test - Wide');
saveas(test_plot, 'tsne_test_wide.png')
